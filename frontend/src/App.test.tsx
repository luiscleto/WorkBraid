...
describe('App', () => {
  ...
  it('retrieves invalid backend-held changes after a browser reload and keeps them correctable', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Keep change' }))
    const immediateChanges = (await screen.findByRole('heading', { name: 'Changes in progress' })).closest('section')
    expect(within(immediateChanges as HTMLElement).getByRole('alert')).toHaveTextContent('Add a title.')
    expect(screen.getByText('Untitled component')).toBeInTheDocument()

    first.unmount()
    render(<App />)
    await submitPath('/tmp/example')
    const reloadedChanges = (await screen.findByRole('heading', { name: 'Changes in progress' })).closest('section')
    expect(within(reloadedChanges as HTMLElement).getByRole('alert')).toHaveTextContent('Add a title.')
    expect(screen.getByText('Untitled component')).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Edit' }))
    expect(screen.getByLabelText('Title')).toHaveAccessibleDescription('Add a title.')
    await user.clear(screen.getByLabelText('Title'))
    await user.type(screen.getByLabelText('Title'), 'Worker')
    await user.click(screen.getByRole('button', { name: 'Keep change' }))
    expect(await screen.findByText('Worker')).toBeInTheDocument()
