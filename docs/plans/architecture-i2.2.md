...
- provide structured **Title** and **Description** authoring. The structured Title is plain human-readable text. A Title edit replaces the complete canonical first H1 block while preserving whether that existing heading is ATX or Setext. A Description edit replaces the exact Markdown body source following that H1. Neither operation changes the component ID or outgoing relationships;
...
- Submit Title-only edits to accepted ATX-H1 and Setext-H1 components. Prove each replaces the complete H1 with the submitted plain title while preserving its existing heading style, the complete frontmatter block byte-for-byte, the exact body bytes, ID, filename, relationships, and regular-file mode.
...
- a Title edit replaces the canonical H1 while preserving its existing ATX/Setext style and exact body bytes; a Description edit replaces the Markdown body while preserving the complete H1 block exactly; neither changes ID or relationships;
...
4. **Authored semantics:** Title replaces H1 while preserving existing ATX/Setext style and exact body bytes; Description replaces the post-H1 body while preserving the complete H1 block; the complete frontmatter remains byte-for-byte exact; ID, path, relationships, unrelated source, and file mode are preserved; title edits do not rename.
...
5. Through structured controls, edit one accepted component's Title and Description. Verify the UI presents it as a change in progress, keeps its accepted identity/relationship implicitly intact, preserves the component's existing ATX or Setext heading style, and does not present the accepted inventory/revision as updated.
