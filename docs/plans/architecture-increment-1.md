...
- Create only the minimal source-root → store-ID association storage.
- Add one explicit backend operation for opening and inspecting a source root.
- Normalize an existing directory path for local lookup without fingerprinting it or treating it as durable repository identity.
...
- Serialize and parse the minimal `architecture.yaml` manifest.
...
The literal v1 format identifier, exact local directory spelling, exact Git command flags, and bootstrap author/message convention are implementation details to pin here. They must satisfy the approved compatibility, isolation, and controlled-Git invariants.
...
- The bootstrap tree contains exactly `architecture.yaml`; `components/` and placeholder files are absent.
...
Real-Git integration tests inspect the bare repository, exact bootstrap tree, manifest, accepted ref, and `HEAD` independence. One representative interrupted-bootstrap state is retried successfully using the same store ID and location. HTTP/frontend tests drive explicit confirmation, incomplete-state display, retry, and the empty accepted view.
...
- Representative failures are explicit: missing `accepted`, malformed or unsupported manifest, and association/manifest store-ID mismatch.
